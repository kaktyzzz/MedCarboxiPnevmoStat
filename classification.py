...
data.pivot_table('№', ['Operirujushhij hirurg'], "Ajerobnost'", 'count').fillna(0).plot(kind='bar', stacked=True)
data.pivot_table('№', ['Vremja operacii'], "Ajerobnost'", 'count').fillna(0).plot()
data.pivot_table('№', ['Vozrast'], "Ajerobnost'", 'count').fillna(0).plot(kind='bar', stacked=True)
data.pivot_table('№', ['Mesjac operacii'], "Ajerobnost'", 'count').fillna(0).plot(kind='bar', stacked=True)
...
# train = data.iloc[:, 2:]
models['RandomForestClassifier'] = RandomForestClassifier(n_estimators = 700) #в параметре передаем кол-во деревьев
...
ROCtrainTRN, ROCtestTRN, ROCtrainTRG, ROCtestTRG = cross_validation.train_test_split(train, target, test_size=0.3)
